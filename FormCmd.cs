...
namespace Celin
{
    ...
    public class FormCmd : BaseCmd
    {
        ...
        public class DefCmd : BaseCmd
        {
            ...
            int OnExecute()
            {
                FormCtx.Current = Id.HasValue ? FormCtx.Select(Id.Parameter) : FormCtx.Current;
                if (FormCtx.Current is null)
                {
                    if (Prompt.GetYesNo("New Form Definition?", true))
                    {
                        PromptOptions();
                        FormCtx.Current = new FormCtx(Id.Parameter);
        ...
        {
            [Argument(0, Description = "Form Context Id")]
            public (bool HasValue, string Parameter) Id { get; }
            int OnExecute()
            {
                ...
                {
                    var ctx = Id.HasValue ? FormCtx.List.Find(f => f.Id.Equals(Id.Parameter)) : FormCtx.Current;
                    if (ctx is null)
                    {
                        Warning("{0} not found!", Id.HasValue ? Id.Parameter : "[Current]");
                    }
                    else
                    {
                        var t = new Task<Tuple<bool, JObject>>(() => ServerCtx.Current.Server.Request<JObject>(ctx.Request));
                        t.Start();
                        while (!t.IsCompleted)
                        {
                            Thread.Sleep(500);
                        ...
                        if (t.Result.Item1)
                        {
                            ResCmd.List.Add(new Response() { Request = ctx, Result = t.Result.Item2 });
